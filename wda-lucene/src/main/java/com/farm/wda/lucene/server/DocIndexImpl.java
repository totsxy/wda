...
public class DocIndexImpl implements DocIndexInter {
	...
	public void indexDoc(DocMap doc) throws Exception {
		...
		Document document = new Document();
		{
			for (String key : doc.keySet()) {
				Field field = new Field(key, doc.getValue(key), doc
						.getStore(key), doc.getIndex(key));
				document.add(field);
			}
		}
	...
	public void deleteFhysicsIndex(String docId) throws Exception {
		...
		log.debug("删除磁盘索引ID:" + docId + ",total time: " + (endTime - startTime)
				+ " ms");
	}
	...
	public void deleteRamIndex(String docId) throws Exception {
		...
		reader.close();
		long endTime = System.currentTimeMillis();
		log.debug("删除内存索引ID:" + docId + ",total time: " + (endTime - startTime)
				+ " ms");
	}
}
